...
class PatchAlignedResizer:
    def __init__(self, patch_size=14):
        self.patch_size = patch_size
        self.original_size = None  # (H_orig, W_orig)
        self.aligned_size = None   # (H_aligned, W_aligned)

    def resize_input(self, x):
        """
        输入 x: Tensor [B, C, H, W]
        返回 patch-aligned 尺寸的图像 [B, C, H', W']
        """
        B, C, H, W = x.shape
        ...
        self.aligned_size = (H_new, W_new)

        if H_new != H or W_new != W:
            x_resized = F.interpolate(x, size=(H_new, W_new), mode='bilinear', align_corners=False)
        else:
            x_resized = x  # 不需要 resize

        return x_resized

    def restore_output(self, y):
        """
        恢复预测结果到原图尺寸
        输入 y: Tensor [B, 1, H', W']（可能是 mask 或 feature）
        返回: [B, 1, H_orig, W_orig]
        """
        if self.original_size is None:
            raise ValueError("You must call resize_input() before restore_output().")

        return F.interpolate(y, size=self.original_size, mode='bilinear', align_corners=False)
...
def visualize_result(image, result, class_names, save_path,  vis_masks=True, conf_thresh=0.001, FP_ind = None, FN_ind = None):
    ...
    for i in range(len(result['boxes'])):
        ...
        # image = draw_box(image, box,f"{class_name}:{score}",color=color)   wjj-0319
        image = draw_box(image, box,  color=color)
        if vis_masks:
...
def extract_peak_points(mask, threshold_ratio=0.3, kernel_size=3, max_points=100):
    assert mask.dim() == 3
    mask = torch.sigmoid(mask.clone().detach())
    max_val = mask.max().item()
    threshold = threshold_ratio * max_val
    pooled = F.max_pool2d(mask, kernel_size, stride=1, padding=(kernel_size-1)//2)
...
def extract_dense_subgroup(coords, max_spacing=7, min_len=3):
    """
    给定一组坐标（已排序），找出所有间距连续小于 max_spacing 的子段（长度 >= min_len）
    """
    coords = np.array(coords)
    dense_groups = []
    start = 0
    for i in range(1, len(coords)):
        if coords[i] - coords[i - 1] >= max_spacing:
            if i - start >= min_len:
                dense_groups.append(coords[start:i])
            start = i
    # 处理最后一段
    if len(coords) - start >= min_len:
        dense_groups.append(coords[start:])
    return dense_groups

#
def filter_aligned_clusters_debug(points, x_tol=3, y_tol=3, max_spacing=7, min_cluster_size=3):
    """
    精准过滤：在一列或一行中提取密集子段，仅保留中点
    """
    ...
    for i in range(N):
        if used[i]: continue
        xi, yi = points[i]
        group_idx = [j for j in range(N) if not used[j] and abs(points[j][0] - xi) <= x_tol]
        if len(group_idx) < min_cluster_size:
            continue
        group = points[group_idx]
        group = group[np.argsort(group[:, 1])]
        ys = group[:, 1]
        xs = group[:, 0]

        segments = extract_dense_subgroup(ys, max_spacing=max_spacing, min_len=min_cluster_size)
        for seg in segments:
            # print(f"\n🧩 提取到竖直子段: {seg.tolist()}")
            y_center = int(seg[len(seg) // 2])
            x_center = int(np.median(xs[np.isin(ys, seg)]))
            # print(f"✅ 保留中点: ({x_center}, {y_center})")
            keep.append((x_center, y_center))
            # 标记被使用的点
            for j in group_idx:
                if points[j][1] in seg:
                    used[j] = True
    ...
    for i in range(N):
        if used[i]: continue
        xi, yi = points[i]
        group_idx = [j for j in range(N) if not used[j] and abs(points[j][1] - yi) <= y_tol]
        if len(group_idx) < min_cluster_size:
            continue
        group = points[group_idx]
        ...
        segments = extract_dense_subgroup(xs, max_spacing=max_spacing, min_len=min_cluster_size)
        for seg in segments:
            # print(f"\n🧩 提取到水平子段: {seg.tolist()}")
            x_center = int(seg[len(seg) // 2])
            y_center = int(np.median(ys[np.isin(xs, seg)]))
            # print(f"✅ 保留中点: ({x_center}, {y_center})")
            keep.append((x_center, y_center))
            for j in group_idx:
                if points[j][0] in seg:
                    used[j] = True
...
def save_image(image, path):
    ...
    Args:
        image (torch.Tensor or np.ndarray): 图像数据，形状支持：
            - 单张图像: (H, W), (1, H, W), (3, H, W)
            - 批量图像: (N, C, H, W)
        path (str): 保存路径。对于批量图像，自动在 path 前缀加 _sample{i}.png

    自动支持：
        - float 图像归一化到 0~255 并转 uint8
        - RGB / 灰度自动判断
    """

    # 如果是 Tensor，先转成 numpy
    ...
    if image.ndim == 4:
        ...
        for i in range(image.shape[0]):
            img_i = image[i]
            if img_i.shape[0] == 3:  # RGB
                img_i = img_i.transpose(1, 2, 0)
            elif img_i.shape[0] == 1:  # 单通道
                img_i = img_i.squeeze(0)
            out_path = f"{os.path.splitext(path)[0]}_sample{i + 1}.png"
            plt.imsave(out_path, img_i, cmap='gray' if img_i.ndim == 2 else None)
...
def normalize_image_to_uint8(image):
    """
    将任意输入图像转换为 uint8 格式，兼容 Tensor / float32 / int 等。
    返回标准 OpenCV 格式图像 (H, W) 或 (H, W, C)
    """
...
def update_and_save_model(
    ...
    """
    通用模型保存函数，支持两种策略：
    1. always_best: 每次 val_loss 更优时保存
    2. interval_best: 每 interval 个 epoch 保存其中最优模型

    Returns:
        float or None: 如果策略为 always_best，返回更新后的 best_loss，否则返回 None。
    """
    os.makedirs(save_path, exist_ok=True)

    if save_strategy == "always_best":
        if best_loss is None or val_loss < best_loss:
            best_loss = val_loss
            timestamp = datetime.now().strftime('%m%d_%H%M')
            model_file = f"{model_name}_epoch{epoch}_{timestamp}.pth"
            model_path = os.path.join(save_path, model_file)

            save_model(model, model_path, epoch, best_loss, optimizer)
            delete_old_models(save_path, model_name, max_models)

            print(f"[AlwaysBest] Saved at epoch {epoch}, val_loss={val_loss:.4f}")
            logging.info(f"[AlwaysBest] Epoch {epoch}, val_loss={val_loss:.4f}")

        return best_loss

    elif save_strategy == "interval_best":
        if cache_dict is None:
            cache_dict = {"best_loss": val_loss, "best_epoch": epoch}
        if val_loss < cache_dict["best_loss"]:
            cache_dict["best_loss"] = val_loss
            cache_dict["best_epoch"] = epoch

        if (epoch + 1) % interval == 0:
            best_epoch = cache_dict["best_epoch"]
            best_loss_val = cache_dict["best_loss"]
            timestamp = datetime.now().strftime('%m%d_%H%M')
            model_file = f"{model_name}_ep{best_epoch}_block{epoch+1}_{timestamp}.pth"
            model_path = os.path.join(save_path, model_file)

            save_model(model, model_path, best_epoch, best_loss_val, optimizer)
            delete_old_models(save_path, model_name, max_models)

            print(f"[IntervalBest] Saved best of epoch {epoch+1-interval}-{epoch+1}: epoch {best_epoch}, val_loss={best_loss_val:.4f}")
            logging.info(f"[IntervalBest] Block {epoch+1-interval}-{epoch+1}: best @ epoch {best_epoch}, val_loss={best_loss_val:.4f}")

            # 重置区间
            cache_dict["best_loss"] = float("inf")
            cache_dict["best_epoch"] = None

        return None

    else:
        raise ValueError(f"Unknown save_strategy: {save_strategy}")


def save_model(model, path, epoch, best_loss, optimizer=None):
    """
    实际保存模型的函数，包括模型参数、epoch、best_loss，以及可选 optimizer。
    """
    checkpoint = {
        'epoch': epoch,
...
def delete_old_models(save_path, model_name, max_models):
    """
    删除模型目录中多余的旧模型文件，只保留最近的 max_models 个。
    """
