...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        #     masks,category_embeddings = classifier.predict(masks)
        # except Exception as e:
        #     print(e)
        #
        # # 利用文本引导，生成文本嵌入
        # text = "people and person"
        ...
        # masks_save_path = os.path.join(temp_dir, "masksfenlei0209")
...
class PixelClassifier(nn.Module):
    def __init__(self, in_channels, num_classes=2, feature_dim=128):
        super(PixelClassifier, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, 16, kernel_size=3, padding=1)
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv2d(16, 8, kernel_size=3, padding=1)
        self.conv3 = nn.Conv2d(8, 1, kernel_size=1)  # 输出类别数
    ...
    def forward(self, x):
        ...
        x = self.relu(x)
        x = self.conv2(x)  # (B, 8, 256, 256)
        x = self.relu(x)
        x = self.conv3(x)  # (B, num_classes, 256, 256)
        return x

    def predict(self, x):
        with torch.no_grad():
            logits = self.forward(x)  # shape: (B, num_classes, 256, 256)

            if self.num_classes > 2:
                probabilities = torch.softmax(logits, dim=1)
                predicted_classes = torch.argmax(probabilities, dim=1)
            else:
                probabilities = torch.sigmoid(logits)  # 取第 1 类的概率
                predicted_classes = (probabilities > 0.5).long()  # 概率 >0.5 为类别 1

            category_embeddings = self.centroids  # shape: (num_classes, feature_dim)

        return logits, category_embeddings
...
def convert_to_grayscale(colored_image):
    ...
    normalized_mask = (gray_image - gray_image.min()) / (gray_image.max() - gray_image.min())
    gray_image = np.clip(normalized_mask, 0, 1)  # Ensuring it's between [0, 1]
    return gray_image
