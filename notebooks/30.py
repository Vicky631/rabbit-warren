...
from utils.utils import PatchAlignedResizer, update_and_save_model
...
class Model(nn.Module):
    ...
    def forward(self, x, points=None, text=None):
        ...
        with torch.no_grad():
            ...
            fg_mask_logit = predict_patchwise_mask_batched(x_dino, self.dino_seg)  # [B, 1, H', W']
...
def test_evaluate(
    ...
    """
    评估模型在验证集上的性能，记录每个 batch 的 F1、Precision、Recall。
    同时保存 denoised、pred、label、kpoint 图。
    """
    model.eval()
    os.makedirs(save_path, exist_ok=True)
    vis_path = os.path.join(save_path, "test_sample")
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(val_dataloader):
            ...
            for b in range(batch_size):
                ...
                # 保存 denoised、pred、label
                denoised_np = denoised_img[b].detach().cpu().numpy()
                pred_np = single_pred.numpy()
                label_np = label[b].detach().cpu().squeeze().numpy()

                save_image(
                    denoised_np, os.path.join(vis_path, f"{full_prefix}_denoised.png")
                )
                save_image(
                    pred_np, os.path.join(vis_path, f"{full_prefix}_pred.png")
                )
                save_image(
                    label_np, os.path.join(vis_path, f"{full_prefix}_label.png")
                )
    ...
    print(f"\n✅ Evaluate Done! mF1: {mean_f1:.4f}, mP: {mean_precision:.4f}, mR: {mean_recall:.4f}")
