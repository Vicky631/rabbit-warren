...
from utils.utils import PatchAlignedResizer, filter_aligned_clusters_debug, save_image
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(val_dataloader):
            ...
            for b in range(batch_size):
                ...
                filename_IMG = os.path.join(log_save_path, f"{filenames[b].split('.')[0]}_pred.png")
                os.makedirs(os.path.dirname(filename_pred), exist_ok=True)  # 确保目录存在
                save_image(single_denoised_img,filename_IMG)
                # 提取并可视化坐标点
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(val_dataloader):
            ...
            for b in range(batch_size):
                ...
                # 提取预测点和真值点
                pred_coordinates1 = extract_coordinates_and_visualize(
                    pred_density=single_pred,
                    save_path=save_path,
                    filename=filename,
                    threshold=0.5,
                ...
                pred_coordinates = filter_aligned_clusters_debug(
                    pred_coordinates1,
                    x_tol=1,
                    y_tol=1,
                    max_spacing=7,
