...
from segment_anything.build_sam_classify import sam_model_registry
...
class Sam_model(nn.Module):
    ...
    def forward(self, x, points, text=None):
        ...
        binary_result, _ = self.sam.mask_decoder(
...
class Model(nn.Module):
    ...
    def forward(self, x, points, text=None):
        ...
        try:
            ...
            print(f"第137行错误: {e}")
            # 可选择抛出异常或返回默认值
            raise RuntimeError("SAM模型执行失败") from e
...
def save_best_model(model, save_path, best_loss, val_loss, epoch, m=100, k=1, optimizer=None, model_name="best_model"):
    """
    ...
    if epoch % m == 0:
        ...
        # 删除旧模型，确保最多保留 k 个模型
        if k > 0:
            delete_old_models(save_path, model_name, k - 1)
...
def evaluate(model, val_dataloader, lossfunc, threshold, device, save_path="outputfilter"):
    """
    评估模型在验证集上的性能，并可视化结果。
