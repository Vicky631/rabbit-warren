...
# 训练函数
def train(predicter, model, train_dataloader, optimizer, lossfunc, threshold, device, epoch, logger, save_path):
    model.train()
    ...
    for batch_idx, data in enumerate(progress_bar):
        ...
        cache = cache_feature(image, predictor, n_shot, debug=False)
        ...
        visualizer = TensorVisualizer(cls_logits)
        visualizer.save_image("/home/zy/wjj/Prompt_sam_localization/output.png")
...
if __name__ == '__main__':
    ...
    # datasets
    sam = sam_model_registry_jj[config['model']['sam_model']](checkpoint='/home/zy/wjj/Prompt_sam_localization/checkpoint/sam_vit_l_0b3195.pth',
    ...
    if model_arch == 'dino':
        ...
        # 这里weights_only=True 原来是这样的
        model.load_state_dict(torch.load('/home/zy/wjj/CrowdSAM-main/weights/dinov2_vitl14_pretrain.pth', weights_only=False))
    ...
    if args.mode == 'test':
        test_data = TestDataset(
            ...
            None,
            os.path.join(args.data_path, 'ground_truth/'),
        ...
        # 测试模型
        f1, mean_precision, mean_recall = test_evaluate(model, test_dataloader, lossfunc, threshold, device)
        ...
        for epoch in range(args.epochs):
            train_loss, train_iou, train_dice = train(predictor,model, train_dataloader, optimizer, lossfunc, threshold, device,
                                                      epoch, logger, args.save_path)
            torch.cuda.empty_cache()
            val_loss, val_iou, val_dice = evaluate(predictor,model, val_dataloader, lossfunc, threshold, device, args.save_path)
