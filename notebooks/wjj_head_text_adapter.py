...
from sympy.codegen.rewriting import optims_scipy
...
class Sam_model(nn.Module):
    def __init__(self, args, model_type, sam_checkpoint):
        ...
        elif args.net == 'sam_adapter':
            from segment_anything.build_sam_adapter import sam_model_registry
            self.sam  = sam_model_registry[model_type](args, checkpoint=sam_checkpoint).to(device)
        # elif args.net == 'sam_fineTuning':
        #     from segment_anything.build_sam_adapter import sam_model_registry
        #     self.sam = sam_model_registry[model_type](args, checkpoint=sam_checkpoint).to(device)
        # elif args.net == 'PromptVit':
...
class Model(nn.Module):
    def __init__(self, args, model_type, sam_checkpoint):
        ...
        self.sam = Sam_model(args, model_type=model_type, sam_checkpoint=sam_checkpoint)
...
def save_best_model(model, save_path, best_loss, val_loss, epoch, optimizer=None, model_name="best_model", max_models=1):
    ...
    if val_loss < best_loss:
        ...
        # Maintain only the latest `max_models` files
        if max_models > 0:
            manage_saved_models(save_path, model_name, max_models)

    return best_loss

def manage_saved_models(save_path, model_name, max_models):
    """
    Manage saved models by keeping only the latest `max_models` files.
    ...
    model_files = sorted(model_files, key=lambda x: os.path.getmtime(os.path.join(save_path, x)), reverse=True)

    # If the number of files exceeds `max_models`, remove the oldest ones
    if len(model_files) > max_models:
        for file_to_remove in model_files[max_models:]:
...
def extract_coordinates(density_map, threshold=0.1):
    """
    从密度图中提取目标点的坐标。

    Args:
        density_map (torch.Tensor): 输入密度图，形状为 (H, W) 或 (1, H, W)。
        threshold (float): 密度图的阈值，用于过滤低值区域。

    Returns:
        list: 坐标点的列表，每个点为 (x, y) 格式。
    """
    if density_map.dim() == 2:  # 如果输入是 (H, W)，添加批次和通道维度
        density_map = density_map.unsqueeze(0).unsqueeze(0)
    elif density_map.dim() == 3:  # 如果输入是 (1, H, W)，添加通道维度
        density_map = density_map.unsqueeze(0)

    # 归一化密度图到 [0, 1]
    max_value = torch.max(density_map).item()
    density_map = density_map / max_value

    # 应用阈值
    density_map[density_map < threshold] = 0

    # 提取非零位置作为坐标
    points = density_map.squeeze().nonzero(as_tuple=True)
...
def freeze_model_parameters(model, unfrozen_keywords=None, fine_tuning_configuration=None):
    """
    动态冻结模型参数。

    Args:
        model (nn.Module): 要冻结的模型或模块。
        unfrozen_keywords (list): 参数名中包含这些关键字的参数将不会被冻结。
        fine_tuning_configuration (list): 用于特定模块配置的解冻。
    """
    unfrozen_keywords = unfrozen_keywords or []

    # 解析 fine_tuning_configuration 中的解冻逻辑（如果提供）
    if fine_tuning_configuration:
        sam_no_freeze_block = [
            f"blocks.{idx}." for idx, flag in enumerate(fine_tuning_configuration) if flag == 1
        ]

    # 遍历模型参数并设置 requires_grad
    for name, param in model.named_parameters():
        # 判断是否解冻
        if any(keyword in name for keyword in unfrozen_keywords):
            param.requires_grad = True
        elif fine_tuning_configuration and any(block in name for block in sam_no_freeze_block):
            param.requires_grad = True
        else:
...
# 训练函数
def train(model, train_dataloader, optimizer, lossfunc, threshold, device, epoch, logger, save_path, fine_tuning_configuration=None):
    model.train()


    # 修改位置 1: 检查 model.sam 是否存在
    if not hasattr(model, 'sam'):
        raise AttributeError("The model does not have a 'sam' attribute.")

    # 调用参数冻结函数
    freeze_model_parameters(
        model=model.sam,  # 只针对 sam 模块操作
        unfrozen_keywords=['mask_decoder', 'Adapter'],  # 不冻结包含这些关键字的模块
        fine_tuning_configuration=fine_tuning_configuration  # 传入特定的 fine-tuning 配置
    )
    ...
    for batch_idx, (image, label, points, text) in enumerate(progress_bar):
        ...
        iou, dice = eval_seg(pred, label, threshold)  # 计算 IoU 和 Dice
        iou_list.append(iou)
        ...
        os.makedirs(log_save_path, exist_ok=True)  # 确保目录存在

        if batch_idx < 3:  # 修改位置 3: 可视化保存的控制逻辑
            save_visualization(
...
def evaluate(model, val_dataloader, lossfunc, threshold, device, save_path="/home/zy/wjj/Prompt_sam_localization/output"):
    ...
    with torch.no_grad():
        for batch_idx, (image, label, points, text) in enumerate(val_dataloader):
            ...
            for b in range(batch_size):
                ...
                # 保存原始密度图
                single_pred1 = pred[b].squeeze(0).cpu().numpy() # 假设 single_pred1 是你的数组
                single_pred1_min = single_pred1.min()
                single_pred1_max = single_pred1.max()

                # 归一化到 [0, 1]
                single_pred1_normalized = (single_pred1 - single_pred1_min) / (single_pred1_max - single_pred1_min)
                # 映射到 [0, 255]
                single_pred1 = (single_pred1_normalized * 255).clip(0, 255).astype(np.uint8) # 移除多余维度，得到 (H, W)
                single_pred1 = (single_pred1 * 255).clip(0, 255).astype(np.uint8)
                filename_pred = os.path.join(save_path, f"{filenames[b].split('.')[0]}_pred.png")
                os.makedirs(os.path.dirname(filename_pred), exist_ok=True)  # 确保目录存在
                cv2.imwrite(filename_pred, single_pred1)
                ...
                f1, precision, recall = calculate_f1_precision_recall(
                    pred_coordinates, true_coordinates, match_threshold=3
                )
            ...
            print(f"F1: {batch_f1:.4f}, Precision: {batch_precision:.4f}, Recall: {batch_recall:.4f}\n")
            # 保存当前 batch 的结果到总文件中
    ...
    # 计算全局平均值
    mean_f1 = np.mean([result["f1"] for result in batch_results])
    mean_precision = np.mean([result["precision"] for result in batch_results])
    mean_recall = np.mean([result["recall"] for result in batch_results])
...
if __name__ == '__main__':
    ...
    if args.mode == 'test':
        ...
        train_data = TrainDataset(
            ...
            is_robustness=True  # 如果需要
        )

        val_data = TestDataset(
            os.path.join(args.data_path, 'valid/images/'),
            os.path.join(args.data_path, 'valid/labels/'),
