...
def extract_coordinates_and_visualize_via_watershed(pred_density_np, save_path, filename, min_area=1, local_max_size=9,enable_dense_patch=None):
    """
    ...
    if enable_dense_patch:
        binary_mask = (binary > 0).astype(np.uint8)
        dense_supp_points = dense_area_with_sparse_limited_supplement(
            binary_mask=binary_mask,
            existing_points=coordinates_a,
            point_count_thresh=20,
            pixel_count_thresh=200,
            min_dist_to_existing=2,
            max_total_supplement=500
        )
        print(f"[密集区域补点] 最终补点数: {len(dense_supp_points)}")
        for (cx, cy) in dense_supp_points:
            cv2.circle(color_img_ws, (cx, cy), 1, (255, 255, 0), -1)  # 黄色补点
            coordinates_a.append((cx, cy))
...
def dense_area_with_sparse_limited_supplement(binary_mask, existing_points,
                                               window_size=32, step=16,
                                               point_count_thresh=25,
                                               pixel_count_thresh=200,
                                               min_dist_to_existing=2,
                                               max_total_supplement=400):
    """
    基于密集区域识别 + 稀疏补点 + 总补点数限制 的补点方法
    """
    h, w = binary_mask.shape
    supplemental_points = []

    existing_points_array = np.array(existing_points)
    existing_tree = cKDTree(existing_points_array)

    selected_tree = None  # 用于记录已补的点位置，做距离检查

    for y in range(0, h - window_size + 1, step):
        for x in range(0, w - window_size + 1, step):
            if len(supplemental_points) >= max_total_supplement:
                break  # 达到补点上限

            x0, y0 = x, y
            x1, y1 = x + window_size, y + window_size

            # 区域内已有点
            in_window = [(cx, cy) for (cx, cy) in existing_points if x0 <= cx < x1 and y0 <= cy < y1]
            if len(in_window) < point_count_thresh:
                continue

            # 区域内白点数
            roi = binary_mask[y0:y1, x0:x1]
            white_pixel_count = np.count_nonzero(roi)
            if white_pixel_count < pixel_count_thresh:
                continue

            # 开始稀疏选点
            ys, xs = np.where(roi > 0)
            candidates = [(x0 + dx, y0 + dy) for dx, dy in zip(xs, ys)]

            for px, py in candidates:
                if len(supplemental_points) >= max_total_supplement:
                    break

                if existing_tree.query_ball_point([px, py], r=min_dist_to_existing):
                    continue
                if selected_tree is not None:
                    if selected_tree.query_ball_point([px, py], r=min_dist_to_existing):
                        continue

                supplemental_points.append((px, py))
                selected_tree = cKDTree(np.array(supplemental_points))  # 更新已选点树

    return supplemental_points
...
def test_evaluate(
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(val_dataloader):
            ...
            # 平均（只对 f1 > 0 的图）
            if valid_f1_count > 0:
                batch_f1 /= valid_f1_count
                batch_precision /= valid_f1_count
                batch_recall /= valid_f1_count
            else:
                batch_f1, batch_precision, batch_recall = 0, 0, 0
    ...
    # ✅ 最终指标，只统计 F1 > 0 的 batch
    valid_batches = [r for r in batch_results if r["f1"] > 0]
    if valid_batches:
        mean_precision = np.mean([r["precision"] for r in valid_batches])
        mean_recall = np.mean([r["recall"] for r in valid_batches])
        mean_f1 = 2 * (mean_precision * mean_recall) / (mean_precision + mean_recall + 1e-6)
    else:
...
if __name__ == '__main__':
    ...
    if args.mode == 'test':
        ...
        val_data = ValDataset(
            ...
            os.path.join(args.data_path, 'labels/'),
            None,
            os.path.join(args.data_path, 'groundtruth/'),
