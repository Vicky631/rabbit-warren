...
def test_evaluate(
    model, val_dataloader, dataname, threshold, device,
    save_path
):
    """
    ...
    """
    model.eval()  # 进入评估模式
    save_path = os.path.join(save_path,"test_sample",dataname)
    ...
    with open(result_file, "w") as f:
        f.write("---threshold:,{threshold},---local_max_size:,{local_max_size},\n")
        f.write("=" * 50 + "\n")
...
if __name__ == '__main__':
    ...
    if args.mode == 'test':
        ...
        # 测试模型
        f1,mean_precision, mean_recall = test_evaluate(model, test_dataloader, args.data_name, threshold, device, args.save_path)
