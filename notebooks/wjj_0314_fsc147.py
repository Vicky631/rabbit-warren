    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(val_dataloader):
            ...
            for b in range(batch_size):
                ...
                #保存原图
                filename_denoised = os.path.join(log_save_path, f"{filenames[b].split('.')[0]}_denoised.png")
                os.makedirs(os.path.dirname(filename_denoised), exist_ok=True)  # 确保目录存在
                single_denoised_img= np.transpose(single_denoised_img.clone.cpu, (1, 2, 0))
                cv2.imwrite(filename_denoised, single_denoised_img)
                print(f"denoised saved to: {filename_denoised}")

                # 保存预测密度图  出现问题可以考虑.clone
