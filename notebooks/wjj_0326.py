...
class Model(nn.Module):
    ...
    def forward(self, x, points=None, text=None):
        ...
        print("[DEBUG] fg_mask max/min:", pred_masks.max().item(), pred_masks.min().item())
        return x, pred_masks
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(val_dataloader):
            ...
            for b in range(batch_size):
                ...
                # print("[DEBUG] fg_mask max/min:", pred[b].max().item(), pred[b].min().item())
                pred[b]= torch.sigmoid(pred[b])
                # 将预测结果限制在 [0, 1] 范围内（代替 sigmoid）
                # pred[b] = torch.clamp(pred[b], 0.0, 1.0)
